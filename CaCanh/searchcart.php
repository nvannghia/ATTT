...
{
   $search=$_POST['search']; 
   $sql = "SELECT * FROM attt.Fish where LOWER(name) LIKE '%$search%'";
if($connection != null)
{
    ...
    try{
       echo'<div class="container">';
        ...
        if(empty($fishInfos))
         echo "<h1 style='color:white;'>Không tìm thấy kết quả của : ".$search."</h1>";
        echo '<div class="row">';
        foreach($fishInfos as $fishinfo)
        {
            ...
            <div class='card' style='width: 18rem; box-shadow:5px 5px 0 rgb(234, 228, 228);'>
                 ...
                 <form action='carts.php' method='post' style='border:none'> 
                    ...
                    <input type='submit' name ='addtocart' class='btn btn-primary button' value ='Thêm vào giỏ hàng'>
